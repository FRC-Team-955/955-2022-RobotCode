...
elevator_motor.Set(ControlMode::PercentOutput, 0);
    ...
    { 
        ...
        else if((limit_switch_top.Get() == 1 || elevator_motor.GetSelectedSensorPosition() > 5000 ) && joystick_position > 0)
        {
            elevator_motor.Set(ControlMode::PercentOutput, 0);
        }
        else if((limit_switch_top.Get() == 1 || elevator_motor.GetSelectedSensorPosition() > 5000 ) && joystick_position < 0)
        ...
        else if((limit_switch_bottom.Get() == 1 || elevator_motor.GetSelectedSensorPosition() < 0) && OffGround())
        {
            elevator_motor.Set(ControlMode::PercentOutput, 0);
        }
        else if((elevator_motor.GetSelectedSensorPosition() < -5000) && joystick_position < 0)
        {
            elevator_motor.Set(ControlMode::PercentOutput, 0);
        }
        else if((elevator_motor.GetSelectedSensorPosition() < -5000) && joystick_position > 0)
        ...
        else if(elevator_motor.GetSelectedSensorPosition() >= -4000 && elevator_motor.GetSelectedSensorPosition() <= 4000)
        {
            elevator_motor.Set(ControlMode::PercentOutput, joystick_position);
        }
        else
